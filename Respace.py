...
def respace(npImage, npSpacing, targetSpacing):
    #npImage = npImage.transpose(1,2,0)
    scaleFactor = npSpacing/targetSpacing
    newShape = (npImage.shape * scaleFactor).astype(np.int)
    rescaled = scipy.ndimage.interpolation.zoom(npImage, scaleFactor)
    return rescaled
